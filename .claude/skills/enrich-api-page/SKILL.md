...
- GramIO contexts use **camelCase** for all properties and methods: `ctx.text`, `ctx.from`, `ctx.chat`, `ctx.replyMessage`, `ctx.senderChat`, etc.
...
| `ctx.replyMessage` | `MessageContext \| undefined` | replied-to message                   |
...
- Don't just repeat what the error says — explain _why it happens_
