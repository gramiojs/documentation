...
// typedoc-vitepress-theme generates an array (one entry per package module).
// We wrap it into an object keyed by "/api/" so VitePress uses it as the API sidebar.
...
try {
	...
	if (Array.isArray(sidebarItems) && sidebarItems.length > 0) {
		// Use a single "/api/" key that covers all API sub-pages
		typeDocSidebar = {
			"/api/": [
				{
					text: "API Reference",
	...
	// Not yet generated — VitePress will use the static fallback in en.locale.ts
}
...
export default defineConfig({
	...
	ignoreDeadLinks: [
		...
		/^\/ru\/telegram\/methods\//,
		/^\/ru\/telegram\/types\//,
		/^\/ru\/telegram(\/|$)/, // RU telegram section index (normalizes to /ru/telegram/index) — not generated yet
		// TypeDoc API reference pages are generated by `bun run gen:typedoc`
		/^\/api\//,
		// TypeDoc-generated relative cross-links — these go through the @gramio/dist/ flattening
		// and may have wrong relative depth; ignore them rather than breaking the build
		// Matches any relative link that navigates up one or more directories: ../  ./../../  etc.
		(link: string) =>
			/^\.\.\//.test(link) ||
			(link.startsWith("./") && link.includes("../")) ||
			link.startsWith("./%5C%3C") ||
			// TypeDoc namespace dirs with encoded brackets or embedded URLs in their name
			link.includes("%5B") ||
			link.includes("(https:"),
	],
	srcExclude: [
	...
	markdown: {
		codeTransformers: [
			...
			transformerTwoslash({
				...
				// Don't throw on Twoslash errors — log a warning and continue building
				throws: false,
				twoslashOptions: {
					compilerOptions: {
						// @ts-expect-error — string is what tsconfig accepts at runtime
						types: ["node"],
					},
				},
