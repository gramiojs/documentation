...
| **Платная** рассылка (`allow_paid_broadcast`) | До **1000 в секунду** (0.1 Stars/сообщение) |
...
### Платный подход: `allow_paid_broadcast`

Если бюджет позволяет, самый простой способ полностью обойти лимиты — параметр `allow_paid_broadcast`. Передайте `true` в любой метод отправки (например, [`sendMessage`](/telegram/methods/sendMessage)), и ваш бот сможет отправлять **до 1000 сообщений в секунду** — по цене **0.1 Telegram Stars за сообщение**, которые автоматически списываются с баланса бота.
...
> [!TIP]
> `allow_paid_broadcast` не заменяет логику повторных попыток — вы всё равно должны обрабатывать временные ошибки. Сочетайте с плагином [auto-retry](/ru/plugins/official/auto-retry) для надёжного решения.

> [!WARNING]
> Перед запуском крупной рассылки убедитесь, что на балансе бота достаточно Stars. Проверить и пополнить баланс можно через [@BotFather](https://t.me/BotFather).
