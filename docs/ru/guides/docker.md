...
> [!TIP]
> Хотите запустить сам **сервер Telegram Bot API** (для загрузок до 2 ГБ и скачивания без лимитов)? См. [руководство по локальному серверу Bot API](/ru/bot-api/local) и официальный образ `ghcr.io/gramiojs/telegram-bot-api`.
