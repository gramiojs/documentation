...
Генерирует кандидатов на юзернейм Telegram-бота с учётом правил BotFather (5–32 символа, `a-z0-9_`, обязательный суффикс `bot`, без ведущей цифры, без подчёркивания в начале/конце, без двух `__` подряд), проверяет занятость через `t.me` по тексту главной кнопки (`Start Bot` / `View Bot` — занято, `Send Message` — свободно) и возвращает ранжированный шорт-лист. Всегда напоминает финально проверить имя в `@BotFather` — свободный на `t.me` юзернейм может быть зарезервирован от ранее удалённого бота.
