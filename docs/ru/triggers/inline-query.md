...
> [!TIP]
> `context.answer()` принимает параметры [`answerInlineQuery`](/ru/telegram/methods/answerInlineQuery) вторым аргументом. Наиболее важный из них — **`cache_time`**: как долго Telegram кэширует результаты на своих серверах (по умолчанию **300 с**). Передайте `cache_time: 0` для динамических результатов, которые меняются с каждым нажатием клавиши — и особенно во время разработки. Все полезные параметры разобраны ниже в разделе [опции `context.answer()`](#opcii-context-answer).

> [!WARNING] Ограничения для превью
...
Большинство пользователей никогда не наберут `@YourBot <запрос>` сами — это трение, о котором они попросту не знают. Решение: спрятать инлайн-запрос **за инлайн-кнопку**. Три кнопки [`switchTo*`](/ru/keyboards/inline-keyboard#switchtochat) телепортируют пользователя прямо в инлайн-режим с **предзаполненным запросом**, а ваш `inlineQuery`-хендлер отдаёт курированный пикер. Один тап, выбор из списка, Telegram сам постит результат — без ввода, без команд.

- [`switchToCurrentChat`](/ru/keyboards/inline-keyboard#switchtocurrentchat) — открывает инлайн-режим *в этом же* чате. Идеально для «выбери одно из вариантов» внутри приватного UI бота.
- [`switchToChat`](/ru/keyboards/inline-keyboard#switchtochat) — предлагает выбрать любой чат и пошарить туда. Канонический flow «Отправить другу» / «Поделиться карточкой».
- [`switchToChosenChat`](/ru/keyboards/inline-keyboard#switchtochosenchat) — то же самое, но с фильтром по типу чата (личка / группа / канал / бот).
...
Второй аргумент `context.answer()` принимает все параметры [`answerInlineQuery`](/ru/telegram/methods/answerInlineQuery). Четыре, которые реально пригодятся:
...
- **`web_app`** — запускает Mini App. См. [гайд по TMA](/ru/tma/overview).
