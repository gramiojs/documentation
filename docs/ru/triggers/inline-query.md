...
> [!TIP]
> `context.answer()` принимает параметры [`answerInlineQuery`](/telegram/methods/answerInlineQuery) вторым аргументом. Наиболее важный из них — **`cache_time`**: как долго Telegram кэширует результаты на своих серверах (по умолчанию **300 с**). Передайте `cache_time: 0` для динамических результатов, которые меняются с каждым нажатием клавиши — и особенно во время разработки.
