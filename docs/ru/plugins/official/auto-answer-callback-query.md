...
### Гарантия ответа при throw (с v0.0.3)

Даже если ваш хэндлер кидает, плагин всё равно дёргает `answerCallbackQuery` — у пользователя не остаётся залипшего спиннера на кнопке. Middleware оборачивает хэндлер в `try/finally`, поэтому авто-ответ срабатывает в `finally` независимо от ошибки:

```ts
bot.callbackQuery("test", async (context) => {
    throw new Error("boom");
    // Плагин всё равно ответит на callback_query —
    // спиннер пропадёт, ошибка уйдёт в bot.onError.
});
```