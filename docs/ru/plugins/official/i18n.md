...
### `localesFor()` — кормим [`bot.syncCommands()`](/ru/triggers/command#метаданные-команд-и-bot-synccommands) переводами

С `@gramio/i18n` v1.5 у инстанса `defineI18n()` появился метод `localesFor(key)` — возвращает `Record<string, string>` со всеми **неосновными** переводами по ключу. Результат кладётся прямо в `CommandMeta.locales`:
...
Под капотом `localesFor` идёт по `Object.keys(languages)`, пропускает primary, гоняет тот же транслятор, что под `ctx.t()`, и выкидывает ключи, разрешившиеся в `null`/отсутствие — частичное покрытие языков работает без танцев с фоллбэками.
