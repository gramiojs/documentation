...
и использовать некоторые метаданные бота. Например, эти метаданные будут использоваться для удаления упоминаний бота в командах.
Если вы это настроите, `GramIO` не будет отправлять запрос `getMe` при запуске.
