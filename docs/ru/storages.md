...
Интерфейс `Storage` в v2.0.x использует дженерик `Data` с типизированными ключами во всех методах (`get`, `set`, `has`, `delete`). Все адаптеры сериализуют ключи через `String()` для обратной совместимости.
