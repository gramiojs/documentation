...
### `localesFor()` — feed [`bot.syncCommands()`](/triggers/command#command-metadata-bot-synccommands) from translations

Since `@gramio/i18n` v1.5, the `defineI18n()` instance exposes a `localesFor(key)` method that returns a `Record<string, string>` of all **non-primary** translations for a given key. Drop the result straight into `CommandMeta.locales`:
...
Internally, `localesFor` iterates `Object.keys(languages)`, skips the primary language, runs the same translator that powers `ctx.t()`, and drops keys that resolve to `null`/missing — so partial language coverage works without a fallback dance.
