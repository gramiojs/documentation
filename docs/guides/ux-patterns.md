---
title: "UX Patterns for Telegram Bots — Button-First Design with GramIO"
head:
    - - meta
      - name: "description"
        content: "Best-practice UX patterns for Telegram bots built with GramIO — button-first navigation, /start anatomy, edit-in-place menus, nested menus with breadcrumbs, toggle buttons, destructive confirmations, loading and empty states, formatting hierarchy, command discovery, and groups vs private chats."
    - - meta
      - name: "keywords"
        content: "telegram bot ux, gramio ux patterns, inline keyboard navigation, button-first bot, /start menu, edit message in place, nested menu, toggle button, destructive confirmation, empty state, setMyCommands, bot design best practices"
---

# UX Patterns

This guide is about **product** decisions, not API surface. The [Keyboards](/keyboards/overview) and [Formatting](/formatting) pages cover *how* to build a keyboard or entity; this page covers *what* to build and *why*.
...
If you find yourself duplicating the same message body between `ctx.send(...)` and `ctx.editText(...)` call sites, graduate to [`@gramio/views`](/plugins/official/views) — its `context.render(view, params)` auto-detects send vs. edit.
...
**One handler per *schema*, not per field.** Share a `CallbackData("toggle").enum("key", […])` across all toggles on a screen; the handler flips `session[key]` and rerenders.
...
For handlers that definitely need a toast, prefer [`@gramio/auto-answer-callback-query`](/plugins/official/auto-answer-callback-query) — it answers empty for you so you never forget.
...
For Mini Apps, replace the default menu button with `bot.api.setChatMenuButton({ menu_button: { type: "web_app", text: "…", web_app: { url } } })` — see [Mini Apps](/tma/overview).
...
And the formatting footguns (full rules in [Formatting](/formatting)):
...
For the full routing map (link pattern → handler → context field), payload encoding rules, the `admin=` permission tokens, and a worked example covering `/start` plus `my_chat_member` admin-rights verification — see [Deep Links](/guides/deep-links).
...
- [Keyboards →](/keyboards/overview) — how to build every keyboard type
- [Formatting →](/formatting) — entities, `format`, the `join` helper
- [Deep Links →](/guides/deep-links) — every `t.me/<bot>?...` link family
- [Views plugin →](/plugins/official/views) — reusable screen templates with auto send/edit
